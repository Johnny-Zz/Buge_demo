...
export function AiParsingOverlayCompact({ 
  ...
  const handleAddAllNew = () => {
    localTasks.forEach(task => {
      if (!isTaskInStore(task) && !isTaskInInbox(task)) {
        addTask(task)
      }
    })
    onSaveToTimeline()
  ...
  const newTasksCount = localTasks.filter(t => !isTaskInStore(t) && !isTaskInInbox(t)).length
            ...
            return (
              <div 
                ...
                className={cn(
                  ...
                  alreadyAdded ? "border-emerald-500/30" : isInbox ? "border-amber-500/30" : "border-white/5"
                )}
              >
